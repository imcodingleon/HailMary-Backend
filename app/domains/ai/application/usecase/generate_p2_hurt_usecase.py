...
class GenerateP2HurtUseCase:
    ...
    async def execute(self, *, user_name: str, ilgan: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "doyoon_p2_hurt AI 실패 → 룰 fallback (user=%s, ilgan=%s, exc=%s)",
                _mask(user_name), ilgan, exc,
            )
            return rule_fallback
        ok, reason = validate_p2_hurt(ai_text, facts)
        if not ok:
            logger.warning(
                "doyoon_p2_hurt 검증 실패 → 룰 fallback (user=%s, ilgan=%s, reason=%s)",
