"""도윤 P-5 ai_charm_index AI 호출 UseCase."""
...
class GenerateP5CharmIndexUseCase:
    def __init__(self, *, ai_client: AIClientPort) -> None:
        self._ai_client = ai_client

    async def execute(self, *, user_name: str, ilgan: str, charm_pct: int) -> str:
        facts = get_doyoon_p5_charm_index_facts(
            user_name=user_name, ilgan=ilgan, charm_pct=charm_pct
        )
        rule_fallback: str = facts["rule_text"]
        try:
            system, user = build_p5_charm_index_prompt(facts)
            ...
            logger.warning("doyoon_p5_charm_index 실패 → 룰 (user=%s, exc=%s)", _mask(user_name), exc)
            return rule_fallback
        ok, reason = validate_p5_charm_index(ai_text, facts)
        if not ok:
            logger.warning("doyoon_p5_charm_index 검증 실패 → 룰 (user=%s, reason=%s)", _mask(user_name), reason)
            return rule_fallback
        return ai_text
