...
class GenerateP4AkyonUseCase:
    def __init__(self, *, ai_client: AIClientPort) -> None:
        self._ai_client = ai_client

    async def execute(
        self, *, user_name: str, ilgan: str, akyon_slot_id: str
    ) -> str:
        facts = get_doyoon_p4_akyon_facts(
            user_name=user_name, ilgan=ilgan, akyon_slot_id=akyon_slot_id
        )
        rule_fallback: str = facts["rule_text"]
        try:
            system, user = build_p4_akyon_prompt(facts)
            ...
            logger.warning(
                "doyoon_p4_akyon AI 실패 → 룰 fallback (user=%s, ilgan=%s, exc=%s)",
                _mask(user_name), ilgan, exc,
            )
            return rule_fallback
        ok, reason = validate_p4_akyon(ai_text, facts)
        if not ok:
            logger.warning(
                "doyoon_p4_akyon 검증 실패 → 룰 fallback (user=%s, reason=%s)",
