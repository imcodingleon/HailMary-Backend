...
from app.domains.ai.domain.service.doyoon_p9_risk_prompt import (
    build_p9_risk_prompt,
    validate_p9_risk,
)
...
class GenerateP9RiskUseCase:
    ...
    async def execute(self, *, user_name: str, ilgan: str) -> str:
        ...
        try:
            ...
            logger.warning("doyoon_p9_risk 실패 → 룰 (user=%s, exc=%s)", _mask(user_name), exc)
            return rule_fallback
        ok, reason = validate_p9_risk(ai_text, facts)
        if not ok:
            logger.warning("doyoon_p9_risk 검증 실패 → 룰 (user=%s, reason=%s)", _mask(user_name), reason)
            return rule_fallback
        return ai_text
