"""도윤 P-6 ai_profile AI 호출 UseCase."""
...
from app.domains.ai.domain.service.doyoon_p6_profile_prompt import (
    build_p6_profile_prompt,
    validate_p6_profile,
)
...
class GenerateP6ProfileUseCase:
    def __init__(self, *, ai_client: AIClientPort) -> None:
        self._ai_client = ai_client

    async def execute(
        self, *, user_name: str, ilgan: str, match_slot_id: str,
        pct_value: int, ohang_lack: str,
    ) -> str:
        facts = get_doyoon_p6_profile_facts(
        ...
        try:
            system, user = build_p6_profile_prompt(facts)
            ...
            logger.warning("doyoon_p6_profile 실패 → 룰 (user=%s, exc=%s)", _mask(user_name), exc)
            return rule_fallback
        ok, reason = validate_p6_profile(ai_text, facts)
        if not ok:
            logger.warning("doyoon_p6_profile 검증 실패 → 룰 (user=%s, reason=%s)", _mask(user_name), reason)
            return rule_fallback
        return ai_text
