"""도윤 P-4 ai_illusion AI 호출 UseCase."""
...
DOYOON_P4_MODEL = "claude-sonnet-4-6"
_MAX_TOKENS = 600
_TEMPERATURE = 0.85
...
class GenerateP4IllusionUseCase:
    def __init__(self, *, ai_client: AIClientPort) -> None:
        self._ai_client = ai_client

    async def execute(self, *, user_name: str, ilgan: str) -> str:
        facts = get_doyoon_p4_illusion_facts(user_name=user_name, ilgan=ilgan)
        rule_fallback: str = facts["rule_text"]
        try:
            system, user = build_p4_illusion_prompt(facts)
            ai_text = await self._ai_client.generate_chapter(
                system_prompt=system,
                user_prompt=user,
                max_tokens=_MAX_TOKENS,
                temperature=_TEMPERATURE,
                model=DOYOON_P4_MODEL,
            )
        except AIClientError as exc:
            logger.warning(
                "doyoon_p4_illusion AI 실패 → 룰 fallback (user=%s, ilgan=%s, exc=%s)",
                _mask(user_name), ilgan, exc,
            )
            return rule_fallback
        ok, reason = validate_p4_illusion(ai_text, facts)
        if not ok:
            logger.warning(
                "doyoon_p4_illusion 검증 실패 → 룰 fallback (user=%s, reason=%s)",
